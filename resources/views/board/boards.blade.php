        ...
        @auth
            <div id="swup" class="navcontainer__flexcontainer navcontainer__flexcontainer--right transition-out_right">
                ...
                <ul id='profile-menu' class="navcontainer__profilecontainer__menu">
                    <li class="navcontainer__profilecontainer__menu__item"><a href="/account" data-no-swup>Account</a></li>
            ...
            @auth
                @if(count($boards) > 0)
                    @foreach($boards as $board)
                        <x-boardcard boardId='{{$board->id}}' owner='{{$board->user_id === auth()->user()->id ? true : false }}' boardtitle='{{$board->title}}' image='{{$board->image}}' leden='{{$board->users_count}}' quotes='{{ $board->quotes_count }}' pinned='{{$board->pivot->pinned}}'></x-boardcard>
                    @endforeach
                @else
