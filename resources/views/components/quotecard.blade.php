@props(['quoteId' => null, 'favourite' => false, 'name' => 'Naam', 'date' => '01-01-2001', 'quote' => 'Lorem Ipsum'])

<div {{ $attributes }} class="quotecard" style="background-color: {{ $favourite == 1 || $favourite == 'true' ? '#FFFCEC' : '#fff' }};">
    <div>
        @if ($favourite == 1 || $favourite == 'true')
            <div class="quotecard__iconcontainer">
                <svg width="40px" height="40px" viewBox="0 1 23 22" fill="#FEE440" xmlns="http://www.w3.org/2000/svg">
        ...
        @endif
        <h3 class="quotecard__quote">“{{ $quote }}”</h3>
    </div>
    <div class="quotecard__infocontainer">
        <div class="quotecard__infocontainer__info">
            <h4> -{{ $name }}, &nbsp</h4>
            <h4> {{ $date }} </h4>
        </div>
        <h4 onClick="loadQuote(`{{ $quoteId }}`, `{{ $favourite }}`, `{{ $quote }}`, `{{ $name }}`, `{{ $date }}`)" class="quotecard__infocontainer__bekijk">Bekijk</h4>
    </div>
</div>